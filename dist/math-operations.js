...
export class MathOperations {
     ...
     */
    static versesInChapter(pericope, chapter) {
        if (chapter <= 0 || chapter > pericope.book.chapterCount)
            return 0;
        let count = 0;
     ...
     */
    static chaptersInRange(pericope) {
        const result = {};
        for (const range of pericope.ranges) {
     ...
     */
    static gaps(pericope) {
        if (pericope.ranges.length === 0 || pericope.isSingleVerse())
            return [];
        const allVersesSet = new Set(pericope.toArray().map((v) => v.toInt()));
    ...
    static continuousRanges(pericope) {
        ...
        groups.push(currentGroup);
        return groups.map((group) => this.createPericopeFromGroup(group, pericope.book));
    }
    /**
     * Returns true if two pericopes share any common verses.
     */
    static intersects(pericope, other) {
     ...
     */
    static contains(pericope, other) {
        if (!this.isValidComparisonTarget(pericope, other))
            return false;
        const myVerses = new Set(pericope.toArray().map((v) => v.toInt()));
        for (const verse of other.toArray()) {
    ...
    static isAdjacentTo(pericope, other) {
        if (!this.isValidComparisonTarget(pericope, other))
            return false;
        ...
        if (!myLast || !otherFirst || !myFirst || !otherLast)
            return false;
    ...
    static precedes(pericope, other) {
        ...
        const myLast = pericope.lastVerse();
        const otherFirst = other.firstVerse();
        if (!myLast || !otherFirst)
            return false;
        return myLast.isBefore(otherFirst);
    }
    static follows(pericope, other) {
        if (!this.isValidComparisonTarget(pericope, other))
            return false;
        const myFirst = pericope.firstVerse();
        const otherLast = other.lastVerse();
        if (!myFirst || !otherLast)
            return false;
        return myFirst.isAfter(otherLast);
    }
    static isValidComparisonTarget(pericope, other) {
        return other instanceof Pericope && pericope.book.equals(other.book);
    }
    static createPericopeFromGroup(group, book) {
        if (group.length === 1) {
