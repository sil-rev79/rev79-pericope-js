...
import { ParseError, InvalidBookError, InvalidChapterError, InvalidVerseError, InvalidRangeError, } from './errors.js';
export class TextProcessor {
     ...
     */
    static parseRanges(rangeText, book, system) {
        const ranges = [];
        ...
        for (const part of parts) {
            context = part.includes('-')
                ...
                : this.parseSingleReference(part, context, book, ranges, system);
        }
        return ranges;
    }
    static parseSingleRange(rangeText, context, book, ranges, system) {
        ...
        if (startPart.includes(':')) {
            ...
            ranges.push(new Range(startChapter, startVerse, endChapter, endVerse));
            ...
            ranges.push(this.versesToRange(1, parseInt(startPart, 10), parseInt(endPart, 10)));
            newContext = 1;
        }
        else if (typeof context === 'number') {
            ranges.push(this.versesToRange(context, parseInt(startPart, 10), parseInt(endPart, 10)));
            newContext = context;
        }
        else {
            ranges.push(this.chaptersToRange(book, parseInt(startPart, 10), parseInt(endPart, 10), system));
    ...
    static parseSingleReference(refText, context, book, ranges, system) {
        let newContext;
        if (refText.includes(':')) {
            ...
            ranges.push(this.versesToRange(1, parseInt(refText, 10), parseInt(refText, 10)));
            newContext = 1;
        }
        else if (typeof context === 'number') {
            ranges.push(this.versesToRange(context, parseInt(refText, 10), parseInt(refText, 10)));
            newContext = context;
        }
        else {
            ranges.push(this.chaptersToRange(book, parseInt(refText, 10), parseInt(refText, 10), system));
            newContext = 'chapter_mode';
        }
        const addedRange = ranges[ranges.length - 1];
        this.validateRange(addedRange, book, system);
    ...
    static versesToRange(chapter, startVerse, endVerse) {
        return new Range(chapter, startVerse, chapter, endVerse);
    }
    static chaptersToRange(book, startChapter, endChapter, system) {
        return new Range(startChapter, 1, endChapter, book.verseCount(endChapter, system));
    }
    static wholeBookRange(book, system) {
        return new Range(1, 1, book.chapterCount, book.verseCount(book.chapterCount, system));
    }
    static validateRange(range, book, system) {
    ...
    static suggestCompletions(input, system = 'english') {
        ...
        if (/^[1-3]$/.test(parts[0]) && parts.length > 1) {
            ...
            const startMatches = allBooks.filter((b) => b.name
                .toLowerCase()
        ...
        if (book) {
            ...
            if (!referenceContent.includes(':')) {
                ...
                for (let i = 1; i <= book.chapterCount; i++) {
                    if (i.toString().startsWith(referenceContent) &&
                ...
                if (book.isValidChapter(chapterNum, system)) {
                    ...
                    if (/^\d+$/.test(versePart)) {
                        ...
                        for (let i = 1; i <= verseCount; i++) {
                            if (i.toString().startsWith(versePart) &&
