...
export class VerseRef {
    ...
    isValid(): boolean {
        if (!this.book.isValid()) return false;
        if (this.chapter <= 0 || this.verse <= 0) return false;
        if (this.chapter > this.book.chapterCount) return false;
    ...
    compareTo(other: VerseRef): number {
        if (this.book.number < other.book.number) return -1;
        if (this.book.number > other.book.number) return 1;

        if (this.chapter < other.chapter) return -1;
        if (this.chapter > other.chapter) return 1;

        if (this.verse < other.verse) return -1;
        if (this.verse > other.verse) return 1;
    ...
    private validateReference(): void {
